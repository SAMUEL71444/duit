-- ============================================================
-- V2 TABLES — semua tabel baru yang dibutuhkan aplikasi
-- PROJECT: tetmhlmosfdrlltkzwec (harus sama dengan .env.local!)
-- URL: https://supabase.com/dashboard/project/tetmhlmosfdrlltkzwec/sql/new
-- Setelah Run, cek di terminal project: npm run db:check
-- ============================================================

-- Verifikasi project (harus muncul 0 baris sebelum migrasi):
...
GRANT ALL ON TABLE subscriptions TO postgres, authenticator, anon, authenticated, service_role;
...
-- Verifikasi akhir (harus muncul 8 baris):
SELECT table_name FROM information_schema.tables
WHERE table_schema = 'public'
  AND table_name IN (
    'portfolio_holdings', 'dca_executions', 'okrs', 'key_results',
    'weekly_reviews', 'prompts', 'subscriptions', 'proposals'
  )
ORDER BY table_name;
